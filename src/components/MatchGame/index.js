...
class MatchGame extends Component {
  ...
  statusChange = () => {
    ...
    if (timeCount !== 0) {
      this.setState(prevState => ({timeCount: prevState.timeCount - 1}))
    } else {
  ...
  render() {
    const {isTrue, activeTabId, scoreCount, timeCount, imgUrl} = this.state
    const thumbnailList = imagesList.filter(
      eachImage => eachImage.category === activeTabId,
    )
    return (
      <div className="app-container">
        <NavBar scoreCount={scoreCount} timeCount={timeCount} />
        {!isTrue && (
        ...
        {isTrue && (
          <ScoreCard scoreCount={scoreCount} playAgain={this.playAgain} />
        )}
      </div>
    )
  }
